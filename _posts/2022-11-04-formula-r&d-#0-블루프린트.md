...
entt은 엔티티를 기본적으로 32비트 `unsigned int` 형의 ID로 취급합니다. 그리고 이 엔티티는 그것을 정의하는 각각의 컴포넌트에 대한 인덱스로서 기능합니다. 
해당 라이브러리는 그 인덱스를 이용해 위 엔티티와 컴포넌트의 CRUD을 효율적으로 할 수 있는 소프트웨어 시스템이라고 할 수 있습니다.
