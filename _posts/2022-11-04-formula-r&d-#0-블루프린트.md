...
자료구조 Sparse Sets을 기반으로 위 데이터 지향형 방식으로 엔티티를 정의한 [entt](https://github.com/skypjack/entt) 라이브러리를 사용합니다.

entt는 엔티티를 기본적으로 32비트 `unsigned int` 형의 ID로 취급합니다. 그리고 이 엔티티는 그것을 정의하는 각각의 컴포넌트에 대한 인덱스로서 기능합니다. 

entt는 그런 엔티티와 컴포넌트의 CRUD을 효율적으로 할 수 있는 소프트웨어 시스템이라고 할 수 있습니다.
...
`entt::registry` 은 entt의 메모리 관리 코어 모듈로서 Sparse Sets에 기반해서 위 시스템을 구현합니다.

보다 자세한 해당 자료구조 구동 방식은 [ECS back and forth](https://skypjack.github.io/2020-08-02-ecs-baf-part-9/) 블로그에 나와있기 때문에 여기서는 그것의 핵심만 파악해본 뒤에 본 프로젝트에 어떻게 활용 될 수 있는지 고민해보도록 하겠습니다.
